...
CWall::CWall(QWidget *parent) : QLabel{parent} {
  int x = parent->width();
  int y = parent->height();
  // 先添加边框
    ...
    for (int j = 1; j < 6; ++j) {
      ...
      for (int k = 0; k < 4; ++k) {
        ...
        if (R) {
          ...
          walls.push_back(QPolygon(QRect(
              QPoint(i * L + adi * 10, j * L + adj * 10),
...
const std::vector<QPolygon> &CWall::getWalls() { return walls; }
