...
myRect::myRect()
    : points{QPoint(0, 0), QPoint(0, 0), QPoint(0, 0), QPoint(0, 0)} {}
...
myRect::myRect(QPoint &&center, int length, int width, int angle) {
  ...
  for (int i = 0; i < 4; i++) {
    QPointF tmpP = QPointF(QPointF(center) + QPointF(d1[i] * double(length),
                                                     d2[i] * double(width)));
    double px = tmpP.rx();
    double py = tmpP.ry();
    points[i] =
        center +
        QPoint((px - double(center.rx())) * cos((double)angle / 180 * pi) -
                   (py - double(center.ry())) * sin((double)angle / 180 * pi),
               (px - double(center.rx())) * sin((double)angle / 180 * pi) +
                   (py - double(center.ry())) * cos((double)angle / 180 * pi));
  }
}
// copy constructor
...
QPoint *myRect::getPoints() { return points; }

CTank::CTank(QWidget *parent, int color_)
    : QLabel(parent), v(TANKVELOCITY), tankRect(), numOfBullets(BULLETSNUM) {
  ...
  image = new QImage;
  if (color_ == GREEN) {
    image->load("://images/greentank.png");
  } else if (color_ == RED) {
    image->load("://images/redtank.png");
  }
}
QImage *CTank::getImage() { return image; }
QString CTank::getRect() {
  return tankRect.getPointstr() + "angle:" + QString::number(angle);
}
QPoint *CTank::getPoints() { return tankRect.getPoints(); }
QPointF CTank::getBulletPos() {
  ...
  for (int i = 0; i < 2; i++) {
    QPointF tmpP =
        QPointF(QPointF(center) + QPointF(d1[i] * 50.0, d2[i] * 30.0));
    double px = tmpP.rx();
    double py = tmpP.ry();
    tmppoints[i] =
        center +
...
int CTank::getcolor() { return color; }
int CTank::getNumOfBullets() { return numOfBullets; }
double CTank::getx() { return x; }
double CTank::gety() { return y; }
double CTank::getangle() { return angle; }
bool *CTank::getKeyPressed() { return keyPressedCondition; }
CTank::~CTank() { delete image; }
...
void CTank::changeNumOfBullets(int delta) { numOfBullets += delta; }
