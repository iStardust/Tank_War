...
Battlefield::Battlefield(QWidget *parent)
    : QWidget(parent), ui(new Ui::Battlefield) {
  ui->setupUi(this);
  ...
  myTank[GREEN]->setPixmap(
      QPixmap::fromImage(*myTank[GREEN]->getImage())
          .transformed(QTransform().rotate(myTank[GREEN]->getangle())));
  myTank[RED]->setPixmap(
      QPixmap::fromImage(*myTank[RED]->getImage())
          .transformed(QTransform().rotate(myTank[RED]->getangle())));
  myTank[GREEN]->setAlignment(Qt::AlignCenter);
  myTank[RED]->setAlignment(Qt::AlignCenter);
  myTank[GREEN]->setGeometry(myTank[GREEN]->getx(), myTank[GREEN]->gety(), 60,
                             60);
  ...
  //子父widget信号传递
  connect(this,SIGNAL(sendmessage()),parent,SLOT(recievemessage()));
}
...
void Battlefield::gameCheck() {
  ...
  if (gameCondition != ONGOING) {
    ...
    QImage explosion("://images/explosion.png");
    QImage greenwin("://images/greenwin.jpg");
    QImage redwin("://images/redwin.jpg");

    bool gameEnd = false;
    if (gameCondition == TIE) {
      myTank[RED]->setPixmap(QPixmap::fromImage(explosion));
      myTank[GREEN]->setPixmap(QPixmap::fromImage(explosion));
      myTank[GREEN]->setGeometry(myTank[GREEN]->getx(), myTank[GREEN]->gety(),
                                 60, 60);
      myTank[RED]->setGeometry(myTank[RED]->getx(), myTank[RED]->gety(), 60,
                               60);
      gameEnd = true;
      ...
      ui->label->setGeometry(0,0,1210,610);
      ui->label->raise();
      greenwin.scaled(ui->label->size(),Qt::KeepAspectRatio);
      ui->label->setScaledContents(true);
      ui->label->setPixmap(QPixmap::fromImage(greenwin));
      ...
      ui->label->setGeometry(0,0,1210,610);
      ui->label->raise();
      redwin.scaled(ui->label->size(),Qt::KeepAspectRatio);
      ui->label->setScaledContents(true);
      ui->label->setPixmap(QPixmap::fromImage(redwin));

      gameEnd = true;
    }
    if(gameEnd){
      ui->buttonExit->setStyleSheet("background-color: rgb(255, 255, 255);color: rgb(255, 0, 0);");
      ui->buttonRestart->setStyleSheet("background-color: rgb(255, 255, 255);color: rgb(0, 255, 0);");
      ui->buttonRestart->show();
      ui->buttonRestart->raise();
      ui->buttonExit->show();
      ui->buttonExit->raise();
    }
  }
...
int Battlefield::checkGame() {
  ...
  for (auto it : myBullets) {
    ...
    QPolygon bulletPo;
    bulletPo << QPoint(bx, by) << QPoint(bx, by + 10)
             << QPoint(bx + 10, by + 10) << QPoint(bx + 10, by);
...
void Battlefield::on_buttonRestart_clicked()
{
  emit sendmessage();
}


void Battlefield::on_buttonExit_clicked()
{
  this->close();
}
